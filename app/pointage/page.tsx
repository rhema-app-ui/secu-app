...
export default function PointagePage() {
  ...
  useEffect(() => {
    const loadTenant = async () => {
      ...
      if (profileRes.data?.tenant_id) {
        setUserTenantId(profileRes.data.tenant_id)
      }
    }
    loadTenant()
  }, [])
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-4 flex flex-col">
      ...
      {serviceState && (
        <div className={`text-center mb-4 p-3 rounded-lg border ${
          ...
          <p className="font-medium">
            {serviceState === "checkin" ? "🟢 Service en cours" : "🔴 Service terminé"}
          </p>
          {serviceState === "checkout" && (
            <button onClick={handleReset} className="mt-2 text-xs underline hover:text-white">
              🔄 Nouveau cycle
            </button>
          )}
        </div>
      ...
      {/* 🔘 Boutons d'action — UTILITAIRES TAILWIND NATIFS (plus de btn-success/btn-danger) */}
      <div className="flex-1 flex flex-col justify-center gap-5">
        {/* 🟢 BOUTON DÉBUT */}
        <button 
          onClick={() => handlePointage("checkin")}
          disabled={loading || gpsStatus === "acquiring" || uploadingPhoto || serviceState === "checkin"}
          className={`w-full py-6 rounded-2xl font-bold text-xl transition-all active:scale-[0.98] ${
            serviceState === "checkin"
